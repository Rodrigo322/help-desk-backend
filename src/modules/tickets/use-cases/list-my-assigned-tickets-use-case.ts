...
export type ListMyAssignedTicketsUseCaseRequest = {
  userId: string;
  status?: TicketStatus;
...
export type ListMyAssignedTicketsUseCaseResponse = {
  items: Array<{
    id: string;
    title: string;
    description: string;
...
export class ListMyAssignedTicketsUseCase {
  constructor(private readonly ticketsRepository: TicketsRepository) {}

  async execute(
    input: ListMyAssignedTicketsUseCaseRequest
  ): Promise<ListMyAssignedTicketsUseCaseResponse> {
    const { items, total } = await this.ticketsRepository.listByAssignedToUser(input.userId, {
      filters: {
        status: input.status,
